...
class MyWin extends JFrame implements ActionListener{
	...
	JTextField txt_sex = new JTextField(15);
	JTextField txt_pid = new JTextField(15);
	JTextField txt_birth = new JTextField(15);
	JTextField txt_tele = new JTextField(15);
	JTextField txt_addr = new JTextField(15);
	
	JTextField txt_sdpt = new JTextField(15);
	...
	JButton btn_sure03 = new JButton("完 成");
	...
	public void actionPerformed(ActionEvent e) {/*******************actionPerformed**********************************/
		if(e.getSource() == btn_sure01) {
			...
			if(username.equals("") || pwd.equals("") || pwd_repeat.equals("")) {
				JOptionPane.showMessageDialog(null,"信息不能位空！");
				flag=false;
			}
			else if(ReaderFunc.hasUser(username)) {
		...
		if(e.getSource() == btn_sure02) {
			...
			Date dBirth = btn_birth.getWhatINeed();
			if(name.equals("") || tele.equals("") || pid.equals("") || addr.equals("")) {
				JOptionPane.showMessageDialog(null,"信息不能位空！");
				flag=false;
			}
			else if(!IDValidate.checkID(pid, dBirth)) {
				JOptionPane.showMessageDialog(null,"身份证不正确或与出生日期不匹配，请重新输入！");
				flag=false;
			}
